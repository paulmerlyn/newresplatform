<?php
/*
prospectinviter.php is script conventionally executed via a cron job. (It can also be executed manually by simply calling this file in my browser.) Its only purpose is to send an initial email invitation to a select number (defined by selection criteria below) of Mediation Platform prospects in the prospects_table, unless that prospect has unsubscribed him/herself via unsubscriber.php (in which case his/her record in prospects_table will have the Unsubscribed column set to 1 (true)). After sending the message to each appropriate prospect, it must also increment the NofMsgs field and record today's date/time in the DofMail field.
*/
...
/* Identify which prospects from prospects_table should receive a marketing solicitation when cron/prospectinviter.php is next run (either manually or via a cron job). (These user-configured settings effect two $query statements below.) */
...
// Formalize the states clause for use in the two $query statements below.
...
// Query the prospects_table table to select various prospect data where NofMsgs is 0 and Unsubscribed != 1.
$query = "SELECT UseFormalTitle, Title, FirstName, LastName, FormMailCode, ProspectEmail, AreaLabel FROM prospects_table WHERE ProspectID >= ".$ProspectIDLow." AND ProspectID <= ".$ProspectIDHigh." AND NofMsgs >= ".$NofMsgsLow." AND NofMsgs <= ".$NofMsgsHigh." AND NofMsgs != 99 ".$theStateClause." AND Unsubscribed != 1";

$result = mysql_query($query) or die('Query (select prospects who have not yet been sent any messages) failed: ' . mysql_error());

while ($row = mysql_fetch_assoc($result))
	{
	echo 'Message sent to: '.$row['FirstName'].' '.$row['LastName'].', '.$row['ProspectEmail'].', '.$row['FormMailCode'].', '.$row['AreaLabel'].'<br>'; // This will either be echoed to the screen (if prospectinviter.php is invoked manually via the browser) or to an email that is sent to me through a setting on the cron jobs section of cpanel (if prospectinviter.php is invoked via a cron job).
...
// Disconnect then reconnect to my mysql database (I have found that the cron job issues a "MySQL server has gone away" error message otherwsies. Reestablishing a DB connection is my attempt to prevent that.
mysql_close($db);
...
$query = "UPDATE prospects_table SET NofMsgs = NofMsgs + 1, DofMail = NOW() WHERE ProspectID >= ".$ProspectIDLow." AND ProspectID <= ".$ProspectIDHigh." AND NofMsgs >= ".$NofMsgsLow." AND NofMsgs <= ".$NofMsgsHigh." AND NofMsgs != 99 ".$theStateClause." AND Unsubscribed != 1";

$result = mysql_query($query) or die('Update to prospects_table for NofMsgs and DofMail failed: ' . mysql_error());

echo '<br><br>The NofMsgs counter and DofMail field in the prospects_table have been updated for each prospect to whom an email was sent<br>'; // This will either be echoed to the screen (if prospectinviter.php is invoked manually via the browser) or to an email that is sent to me through a setting on the cron jobs section of cpanel (if prospectinviter.php is invoked via a cron job).
