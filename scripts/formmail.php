<?php
/* I injected this session start function and another code block into the formmail.php native script around lines 2650 for Securimage CAPTCHA here (see http://www.phpcaptcha.org/documentation/quickstart/). Previously, I had tried in vain to implement formmail.php's native CAPTCHA-type system ("image verify") as proposed by formmail.php's guardian Tectite. I'd also tried and failed to implement White Hat's CAPTCHA solution (see www.white-hat-web-design.co.uk/articles/php-captcha.php), which worked for me inside mediationcareersform.shtml. The Securimage CAPTCHA works great with formmail.php, thankfully!  */
 ...
 * setting.
 *
 * I have modified the following: constant EMAIL_NAME (addition of underscore), $TARGET_EMAIL, $DEF_ALERT, $LOGDIR, and $TEMPLATEDIR, and $REQUIRE_CAPTCHA, $aMessages[MSG_ARESP_NO_MATCH]
 *
 *****************************************************************************/

            /* Help: http://www.tectite.com/fmdoc/email_name.php */
define("EMAIL_NAME","^[-a-zA-Z_0-9.]+");    // the '^' is an important security feature! (Note that I added the underscore '_' (between the Z and the zero( b/c I think it may have been omitted inadvertently.)

/* The next 30 lines of code or so are largely my custom adaptation in order to ensure that the array TARGET_EMAIL() contains every possible email address domain of a mediator in the database so all such people can receive the form results. */
...
// Select email stubs (i.e. the part of the email address that comes after the '@' symbol) for every email address (Email, ProfessionalEmail, and EmailPersonal) in both the mediators_table and mediators_table_demo. Replace the period(s) in each stub with a '\.' (note need for triple '\' to effect this replacement).
$query = "SELECT REPLACE(SUBSTRING_INDEX(Email,'@',-1),'.','\\\.') AS EmailStub, REPLACE(SUBSTRING_INDEX(ProfessionalEmail,'@',-1),'.','\\\.') AS ProfessionalEmailStub, REPLACE(SUBSTRING_INDEX(EmailPersonal,'@',-1),'.','\\\.') AS EmailPersonalStub FROM mediators_table UNION SELECT REPLACE(SUBSTRING_INDEX(Email,'@',-1),'.','\\\.') AS EmailDemoStub, REPLACE(SUBSTRING_INDEX(ProfessionalEmail,'@',-1),'.','\\\.') AS ProfessionalEmailDemoStub, REPLACE(SUBSTRING_INDEX(EmailPersonal,'@',-1),'.','\\\.') AS EmailPersonalDemoStub FROM mediators_table_demo";
$result = mysql_query($query) or die('The SELECT query count of email addresses has failed i.e. '.$query.' failed: ' . mysql_error());
$emailsArray = array(); // Initialize $emailsArray()
while ($row = mysql_fetch_assoc($result))
	{
	array_push($emailsArray, $row['EmailStub']);
	array_push($emailsArray, $row['ProfessionalEmailStub']);
	array_push($emailsArray, $row['EmailPersonalStub']);
	array_push($emailsArray, $row['EmailDemoStub']);
	array_push($emailsArray, $row['ProfessionalEmailDemoStub']);
	array_push($emailsArray, $row['EmailPersonalDemoStub']);
	}
$emailsArray = array_unique($emailsArray); // Remove duplicates

$emailsArray = array_filter($emailsArray); // Nice way to remove null, empty, and false (0) values from an array. Reference http://briancray.com/2009/04/25/remove-null-values-php-arrays. But note that it also removes 0 (i.e. false) values.

	           /* Help: http://www.tectite.com/fmdoc/target_email.php */
$TARGET_EMAIL = array();
foreach ($emailsArray as $item)
	{
	array_push($TARGET_EMAIL, EMAIL_NAME.'@'.$item.'$'); // Here I populate the $TARGET_EMAIL() array by prepending and appending to elements in $emailsArray. The goal is to make $TARGET_EMAIL elements conform to the format indicated in http://www.tectite.com/fmdoc/target_email.php.
	}
	
/* This marks the end of my custom adaptation to define $TARGET_EMAIL(). */

            /* Help: http://www.tectite.com/fmdoc/target_email.php */
//$TARGET_EMAIL = array(EMAIL_NAME."@yourhost\.com$");

            /* Help: http://www.tectite.com/fmdoc/def_alert.php */
define("DEF_ALERT","paul@newresolutionmediation.com");
...
$LOGDIR = "/home/paulme6/public_html/nrmedlic/form_logs"; // directory for log files; empty string to
                                        // disallow log files
...
$TEMPLATEDIR = "/home/paulme6/public_html/nrmedlic/form_templates";  // directory for template files; empty string
...
function LoadBuiltinLanguage()
{
    ...
    $aMessages[MSG_OPEN_TEMPLATE] = 'Failed to open template "$NAME" $MNUM: $ERROR';
        ...
        // Parameters: none
    $aMessages[MSG_ERROR_PROC] = 'Oops! An error occurred while processing the '.
                                 'form $MNUM...\n\n';
    ...
    $aMessages[MSG_NO_DATA_PAGE] =  'This URL is a Form submission program.\n'.
        ...
        // Parameters: none
    $aMessages[MSG_REQD_ERROR] = 'The form required some values that you did not provide. Please see details below\nthen use your browser\'s Back button to correct the error(s):\n';
    ...
    $aMessages[MSG_VERIFY_MISSING] = 'Image verification string missing. This'.
        ...
        // imgverify option
        // Parameters: none
    $aMessages[MSG_VERIFY_MATCH] = "To help us fight spam, please use your browser's back button to return to the previous page. Enter the security code (a randomly generated sequence of letters) before clicking the 'Submit' button. Thank you.";
...
/* I'm going to try inserting code from Securimage CAPTCHA here (see http://www.phpcaptcha.org/documentation/quickstart/) */
...
if ($securimage->check($_POST['captcha_code']) == false) 
  ...
  echo 'To help protect our mediators from spam, we need you to correctly enter the security code before submitting your form.<br>Please use your browser\'s Back button to try again.';
  ob_flush();
  exit;
	}
	...
	{
	/* Here's a custom autorespond routine that I wrote. [Note that formmail.php requires Captcha to be used in order for formmail's own autoresponse functionality to operate. Since I couldn't get formmail's own CAPTCHA implementation to work for me, I had to use a third-party CAPTCHA (Securimage CAPTCHA) and have hence used my own custom autoresponse system, not formmail's native autoresponse feature. */

	// Create short variable names
	$PartyName = $_POST['realname'];
	$PartyEmail = $_POST['email'];
	$FromEmail = 'donotreply@newresolutionmediation.com';

	// First send autorespond email to the party who filled out c_contact.shtml, c_consultreqform.shtml, c_otherpartyrequestform.shtml, c_scheduapptform.shtml, or otherpartyrequestform.shtml
	$message = "Hello $PartyName\n\nThis is an automated reply just to let you know we've received your request. Please don't reply directly to this message. One of our independent mediators will be in touch soon.\n\n";
	$message .= "In the meantime, thank you for your request.\n\n";
	$message .= "Sincerely\n\n";
	$message .= "New Resolution LLC\n";
	$to = $PartyEmail;
	$subject = 'Request Received - Thank You';
	$headers = "From: $FromEmail"."\r\n" . 'X-Mailer: PHP/' . phpversion();
	mail($to,$subject,$message,$headers);

	// End of my custom autoresponse code.
	}
// End of Securimage CAPTCHA code injection.
