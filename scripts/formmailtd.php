<?php
/*
formmailtd.php is called by c_schedmediationformtd.php. It's similar to formmail.php (which is called by the five main client-level forms (i.e. c_schedmediationform.shtml, c_contact.shtml, consultreqform.shtml, c_inpersonconsultreqform.shtml, and otherpartyrequestform.shtml). The reason why c_schedmediationformtd.php needs this customized version (i.e. formmailtd.php) is because, for security reasons, formmail.php assigns to $TARGET_EMAIL only email addresses (i.e. values of the form's "recipients" field) of mediators in the DB. Since the value of the recipients field for c_schedmediationformtd.php is obtained via emailtdform.php and could be any email address entered by a test driver of the appointment forms utility, formmailtd.php cannot apply the same level of scrutiny in its assignment of $TARGET_EMAIL.
*/
...
define("EMAIL_NAME","^[a-zA-Z0-9_\-.]+@[a-zA-Z0-9_\-]+\.[a-zA-Z0-9_\-.]+$");
//define("EMAIL_NAME","^[-a-zA-Z_0-9.]+");    // the '^' is an important security feature! (Note that I added the underscore '_' b/c I think it may have been omitted inadvertently.)
//define("TOP_LEVEL_DOM","^[-a-zA-Z_0-9.]+");    // the '^' is an important security feature! (Note that I added the underscore '_' b/c I think it may have been omitted inadvertently.)

            /* Help: http://www.tectite.com/fmdoc/target_email.php */
$TARGET_EMAIL = array(EMAIL_NAME); // I'm pretty much creating a wildcard here, accepting any email address of form anything@anything
