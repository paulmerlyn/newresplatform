<?php
/* 
This script is the slave processor for admin9.php, which allws the administrator to (i) add new username-password (non-encrypted) pairs to either the userpass_table or the userpass_table_demo, and (ii) to look up usernames/passwords for a given password/username. 
*/
// Start a session
session_start();
...
<title>Processing Slave Script for admin9.php</title>
...
// Decide on which username-password table name.
switch ($_SESSION['ClientDemoSelected'])
{
	...
	case 'demo' :
		...
		break;
	default :
		echo 'Error: Unable to determine the name of an appropriate username-password table (client or demo) in admin10.php.<br>';
}

// Establish a DB connection
$db = mysql_connect('localhost', 'paulme6_merlyn', 'fePhaCj64mkik')
or die('Could not connect: ' . mysql_error());
mysql_select_db('paulme6_newresolution') or die('Could not select database in admin10.php: ' . mysql_error());
...
if ($NofUPpairs >= 1) // Don't bother to formulate a query if there are no username-password pairs to insert.
	{
	$query = "INSERT IGNORE INTO ".$userpasstablename." (Username, Password, Available) VALUES ";
	...
	$result = mysql_query($query) or die('Either you are not authenticated to insert Username, Password, and Available into the userpass_table or userpass_table_demo, or the insertion into the database failed. ' . mysql_error());
	}

if ($NofUPpairs >= 1)
	{
?>
	<p class='basictext' style='margin-left: 50px; margin-right: 50px; margin-top: 40px; font-size: 14px;'>The following username(s) and password(s) were submitted to the <?=$_SESSION['ClientDemoSelected'];?> database. (Note: Usernames or passwords that are <em>duplicates</em> of usernames or passwords already stored in the <?=$_SESSION['ClientDemoSelected'];?> database will not be inserted into the database.)</p><br><br>
...
<?php
	...
	{
?>
	<p class='basictext' style='margin-left: 50px; margin-right: 50px; margin-top: 40px; font-size: 14px;'>No valid username-password pairs were submitted for insertion into the <?=$_SESSION['ClientDemoSelected'];?> database.</p><br>
<?php
	}
}
...
{
$query = "SELECT count(*) FROM ".$userpasstablename." WHERE Password = '".$Password."'";
...
switch ($NofMatches)
	{
	case 0:
		echo '<p class=\'basictext\' style=\'margin-left: 50px; margin-right: 50px; margin-top: 40px; font-size: 14px;\'>You entered the password <span style="font-family: Courier, Times, serif;">'.$Password.'</span>. Unable to look up the username for this password. No such password found in the '.$_SESSION['ClientDemoSelected'].' database.</p>';
		break;
	case 1: // Now look up the Username for the matching Password.
		$query = "SELECT Username, Password FROM ".$userpasstablename." WHERE Password = '".$Password."'";
		...
		break;
	default:
		echo 'Warning: The '.$userpasstablename.' apparently has duplicate entries as detected by the switch statement in admin10.php.';
		break;
	}
...
{
$query = "SELECT count(*) FROM ".$userpasstablename." WHERE Username = '".$Username."'";
...
switch ($NofMatches)
	{
	case 0:
		echo '<p class=\'basictext\' style=\'margin-left: 50px; margin-right: 50px; margin-top: 40px; font-size: 14px;\'>You entered the username <span style="font-family: Courier, Times, serif;">'.$Username.'</span>. Unable to look up the password for this username. No such username found in the '.$_SESSION['ClientDemoSelected'].' database.</p>';
		break;
	case 1: // Now look up the Password for the matching Username.
		$query = "SELECT Username, Password FROM ".$userpasstablename." WHERE Username = '".$Username."'";
...
<input type='button' name='Continue' class='buttonstyle' value='Continue' onclick='javascript: window.location = "admin9.php";'> <!-- This is not a submit button and functions independenly of the action='unwind.php' form -->
</td>
<td>&nbsp;</td>
<td width="120" style="text-align: left;">
